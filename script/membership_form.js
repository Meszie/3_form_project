...
function showTab(n) {
  ...
  if (currentTab === x.length - 1) {
    ...
    nextBtn.onclick = function () {
      ...
      // Set the flag to indicate the submission process is starting
      isSubmitting = true;

      // Check if the Submit button is explicitly clicked
      if (validateTab()) {
        Swal.fire({
          title: "Are you sure?",
          ...
          if (result.isConfirmed) {
            Swal.fire({
              ...
              icon: "success",
              didOpen: () => {
                // Add a custom class to the dialog content
          ...
          // Reset the flag after the submission process is complete
          isSubmitting = false;
        });
      }
    };
